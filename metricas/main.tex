...
Un sistema de seguridad puede procesar diferentes tipos de entradas (trafico de red, carga de memoria, logs, tipo de sistema operativo, puertos abiertos, etc). Existen además, diferentes tipos de sistemas de seguridad (antivirus, firewalls, IDS, IPS, etc.) los cuales, de acuerdo a unas entradas determinadas realizan tareas para proteger, desde equipos de computo hasta redes de posibles ataques. Por otro lado, el tipo de sistema que se implemente es quien dictará el tipo de entradas que se usen; asi por ejemplo, las entradas para un antivirus no serán las mismas que para un sistema de detección de intruciones (IDS). Lo anterior plantea una primera restricción ¿Que sistema de seguridad se va a implementar?

Para el caso, se hará enfasis en los IDS el cual es un sistema que evalua el tráfico de red en busqueda de amenazas y riesgos y lanza alarmas en caso de deteccion de un patrón de tráfico anormal. Como para la evaluación y test de un IDS es necesario definir el trafico de entrada (restringido a trafico DoS y trafico normal para el caso), ¿como generar este tipo de trafico de manera controlada para evaluar bajo condiciones controladas un IDS? 
